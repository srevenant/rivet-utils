defmodule Rivet.Utils.Cli do
  ...
  def run_command(args, opts) do
    ...
    with {matched, [cmd | args], []} <- OptionParser.parse(args, opts_args),
         {:ok, module} <- match_command(cmd, commands) do
      case apply(module, :run, [opts, matched, args]) do
        :ok -> :ok
        other -> die("Invalid result from run (#{module}.run/3): #{inspect(other)}")
      end
    else
      {[], [], []} -> syntax(opts, "Unmatched command")
  ...
  def list_option(opt, _optkey, type, _aliases, _info) do
    [:bright, "--#{opt}", :reset, "=#{to_string(type) |> String.upcase()}\n"]
  end
  ...
  defp match_command(_cmd, _, []), do: false
end
